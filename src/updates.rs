...
pub struct CallbackQuery {
    #[pyo3(get)] pub query_id:   i64,
    #[pyo3(get)] pub user_id:    i64,
    #[pyo3(get)] pub message_id: Option<i32>,
    #[pyo3(get)] pub chat_id:    Option<i64>,
    #[pyo3(get)] pub data:       Option<String>,
...
impl CallbackQuery {
    ...
    fn answer<'py>(&self, py: Python<'py>, text: Option<String>, alert: bool) -> PyResult<Bound<'py, PyAny>> {
        let c   = Arc::clone(&self.client);
        let qid = self.query_id;
        future_into_py(py, async move {
            c.answer_callback_query(qid, text.as_deref(), alert).await.map_err(py_err)?;
            Ok(())
        })
    }
}
...
pub struct MessageDeletion {
    #[pyo3(get)] pub message_ids: Vec<i32>,
    #[pyo3(get)] pub channel_id:  Option<i64>,
}

#[pymethods]
impl MessageDeletion {
    fn __repr__(&self) -> String {
        format!("MessageDeletion(ids={:?}, channel_id={:?})", self.message_ids, self.channel_id)
    }
}
...
pub struct InlineQuery {
    ...
    #[pyo3(get)] pub offset:   String,
    #[pyo3(get)] pub peer_id:  Option<i64>,
}

#[pymethods]
impl InlineQuery {
    fn __repr__(&self) -> String {
        format!("InlineQuery(user_id={}, query={:?})", self.user_id, self.query)
    }
}
...
pub struct InlineSend {
    #[pyo3(get)] pub user_id:   i64,
    #[pyo3(get)] pub query:     String,
    #[pyo3(get)] pub result_id: String,
}

#[pymethods]
impl InlineSend {
    fn __repr__(&self) -> String {
        format!("InlineSend(user_id={}, result_id={:?})", self.user_id, self.result_id)
    }
}
...
impl UserStatus {
    fn __repr__(&self) -> String {
        format!("UserStatus(user_id={}, status={:?})", self.user_id, self.status)
    }
}

#[pyclass]
pub struct ChatAction {
    #[pyo3(get)] pub peer_id: i64,
    #[pyo3(get)] pub user_id: i64,
    // "typing" | "upload_photo" | "record_video" | "cancel" | ... | "other"
    #[pyo3(get)] pub action:  String,
}

#[pymethods]
impl ChatAction {
    fn __repr__(&self) -> String {
        format!("ChatAction(user_id={}, action={:?})", self.user_id, self.action)
    }
}
...
pub struct ParticipantUpdate {
    ...
    #[pyo3(get)] pub user_id:    i64,
    #[pyo3(get)] pub actor_id:   i64,
    #[pyo3(get)] pub date:       i32,
    // true = channel/supergroup, false = basic group
    #[pyo3(get)] pub is_channel: bool,
}

#[pymethods]
impl ParticipantUpdate {
    fn __repr__(&self) -> String {
        format!("ParticipantUpdate(chat_id={}, user_id={})", self.chat_id, self.user_id)
    }
}
...
impl JoinRequest {
    fn __repr__(&self) -> String {
        format!("JoinRequest(user_id={}, peer_id={})", self.user_id, self.peer_id)
    }
}
...
pub struct MessageReaction {
    #[pyo3(get)] pub peer_id:       i64,
    #[pyo3(get)] pub msg_id:        i32,
    #[pyo3(get)] pub date:          i32,
    #[pyo3(get)] pub actor_id:      i64,
    #[pyo3(get)] pub old_reactions: Vec<String>,
    #[pyo3(get)] pub new_reactions: Vec<String>,
}

#[pymethods]
impl MessageReaction {
    fn __repr__(&self) -> String {
        format!("MessageReaction(msg_id={}, new={:?})", self.msg_id, self.new_reactions)
    }
}

#[pyclass]
pub struct PollVote {
    #[pyo3(get)] pub poll_id:   i64,
    #[pyo3(get)] pub peer_id:   i64,
    #[pyo3(get)] pub positions: Vec<i32>,
}

#[pymethods]
impl PollVote {
    fn __repr__(&self) -> String {
        format!("PollVote(poll_id={}, peer_id={})", self.poll_id, self.peer_id)
    }
}
...
pub struct BotStopped {
    #[pyo3(get)] pub user_id: i64,
    #[pyo3(get)] pub date:    i32,
    #[pyo3(get)] pub stopped: bool,
}

#[pymethods]
impl BotStopped {
    fn __repr__(&self) -> String {
        format!("BotStopped(user_id={}, stopped={})", self.user_id, self.stopped)
    }
}
...
pub struct RawUpdate {
    #[pyo3(get)] pub constructor_id: u32,
    #[pyo3(get)] pub type_name:      String,
}

#[pymethods]
impl RawUpdate {
    fn __repr__(&self) -> String {
        format!("RawUpdate(cid=0x{:08x}, type={:?})", self.constructor_id, self.type_name)
    }
}
...
pub fn update_to_py(
    ...
    match upd {
        ...
        ferogram::update::Update::MessageEdited(m) => {
            ok!("edited_message", from_incoming(m, Some(Arc::clone(&client))))
        }
        ferogram::update::Update::MessageDeleted(d) => {
            ok!("message_deleted", MessageDeletion {
        ...
        ferogram::update::Update::CallbackQuery(q) => {
            ...
            ok!("callback_query", CallbackQuery {
                query_id:   q.query_id,
                user_id:    q.user_id,
        ...
        ferogram::update::Update::ParticipantUpdate(p) => {
            ok!("participant_update", ParticipantUpdate {
                chat_id:    p.chat_id,
                user_id:    p.user_id,
                actor_id:   p.actor_id,
                date:       p.date,
        ...
        ferogram::update::Update::MessageReaction(r) => {
            ok!("message_reaction", MessageReaction {
                peer_id:       peer_to_id(&r.peer),
                msg_id:        r.msg_id,
                date:          r.date,
